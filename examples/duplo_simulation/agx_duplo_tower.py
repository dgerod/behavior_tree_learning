"""
Simulation environment for stacking duplo bricks as a tower
"""
import sys
...
def build_scene(rosid=10, visual=True):
    ...
    x_pos = [-0.05, 0, 0.05, -0.05, 0, 0.05, -0.05, 0, 0.05]
    y_pos = [-0.1, -0.1, -0.1, -0.05, -0.05, -0.05, 0, 0, 0]
    z_pos = [0.000001] * 9
    start_positions = []

    #Goal positions, uncomment for testing
    #x_pos = [0.0, 0.0, 0.0, -0.05, 0, 0.05,-0.05, 0, 0.05 ]
    #y_pos = [0.05, 0.05, 0.05, -0.05, -0.05, -0.05, 0, 0, 0]
    #z_pos = [0.0, 0.0192, 2*0.0192, -0.05, -0.05, -0.05, 0, 0, 0]
    ...
    scene_settings.listener_pool = common_functions.ListenerPool(size=12)
