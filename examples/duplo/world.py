...
class WorldSimulator(implements(World)):
    ...
    def place(self, position=None, brick=None):
        ...
        if (self.state.picked is not None
                and (position is not None or brick is not None)):
    ...
    def on_top(self, upper_brick, lower_brick):
        ...
        if (abs(upper_brick.x - lower_brick.x) < self.sm_par.ontop_margin
                and abs(upper_brick.y - lower_brick.y) < self.sm_par.ontop_margin
                and (0 < upper_brick.z - lower_brick.z <= self.sm_par.brick_height + self.sm_par.ontop_margin)):
            return True
        return False
